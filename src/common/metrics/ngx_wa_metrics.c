    ...
    ngx_rbtree_node_t *sentinel)
{
    ...
    if (ngx_wa_metrics_add(metrics, &n->key.str, m->type, &mid) != NGX_OK) {
...
ngx_wa_metrics_alloc(ngx_cycle_t *cycle)
{
    ...
    if (metrics == NULL) {
        ngx_wasm_log_error(NGX_LOG_ERR, cycle->log, 0,
                           "failed allocating metrics structure");
    ...
    if (metrics->shm == NULL) {
        ngx_wasm_log_error(NGX_LOG_ERR, cycle->log, 0,
                           "failed allocating metrics shm structure");
...
ngx_int_t
ngx_wa_metrics_add(ngx_wa_metrics_t *metrics, ngx_str_t *name,
