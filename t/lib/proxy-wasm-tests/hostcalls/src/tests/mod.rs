...
pub(crate) fn test_define_metrics(ctx: &mut (dyn TestContext + 'static), phase: TestPhase) {
    ...
    for metric in metrics_config.split(",") {
        ...
        for i in 1..(n + 1) {
            let mut name = format!("{}{}_{:?}", metric_char, i, phase);
            ...
            info!("defined metric {} as {:?} at {:?}", &name, m_id, phase);
